 ...
 */(function($, f) {
	var Unslider = function() {
		...
		_.o = {
			...
			starting: f,    // invoke 2016 animation (function with argument)
			...
			if ($.event.special['move'] || $.Event('move')) {
				el.on('movestart', function(e) {
					...
					if (Math.abs(left) > 30){var i = left > 0 ? _.i-1 : _.i+1;
						if (i < 0 || i >= len) i = _.i;
						_.to(i);
					}else{
						_.to(_.i);
		...
		_.to = function(index, callback) {
			...
			var o = _.o,
				...
				li = _.li,
				current = _.i,
				target = li.eq(index);$.isFunction(o.starting) && !callback && o.starting(el, li.eq(current), index);//  To slide or not to slide
			...
			target = li.eq(index);var speed = callback ? 5 : o.speed | 0,
				...
				el.find('.dot').eq(index).addClass('active').siblings().removeClass('active');el.animate(obj, speed, easing) && ul.animate($.extend({left: '-' + index + '00%'}, obj), speed, easing, function(data) {
					_.i = index;$.isFunction(o.complete) && !callback && o.complete(index, el, target);
				});
			};
